...
// Distributed Prefix & Id
const distributed = process.env.CLICKHOUSE_DISTRIBUTED || false;
const clusterId = process.env.CLICKHOUSE_CLUSTERID || false;
...
class TimeoutThrottler {
    ...
    start() {
        ...
        setTimeout(async () => {
			...
			if(debug){ console.log(e.response.status, e.response.data) }
                    } else {
                        console.log(e)
                    }
                }
...
/* Initialize */
var initialize = function(dbName, skip){
	var state = 4;
	...
	ch.query(dbQuery, undefined, async function (err, data) {
		...
		/* Storage policy for S3 */
		if (storage_policy){
			if (debug) console.log('ALTER storage policy', storage_policy);
		...
		/* Create distributed table (testing) */
		if (distributed && clusterId){

			var distName = distributed+"_"+dbName;
			var ts_ds_table = "CREATE TABLE IF NOT EXISTS "+distName+"_time_series (date Date, fingerprint UInt64, labels String, name String) ENGINE = Distributed('"+clusterId+"','"+dbName+"','time_series',fingerprint);"
			var sm_ds_table = "CREATE TABLE IF NOT EXISTS "+distName+"_samples (fingerprint UInt64, timestamp_ms Int64, value Float64, string String) ENGINE = Distributed('"+clusterId+"','"+dbName+"','samples',fingerprint);"

		  	await ch.query(ts_ds_table, undefined, function(err,data){
				if (err) { console.log(err); process.exit(1); }
				else if (debug) console.log('Distributed Timeseries Table ready!', distName, clusterId);
				return true;
			});
		  	await ch.query(sm_ds_table, undefined, function(err,data){
				if (err) { console.log(err); process.exit(1); }
				else if (debug) console.log('Distributed Samples Table ready!', distName, clusterId);
				return true;
			});
		}
