...
var initialize = function(dbName, skip){
	...
	ch.query(dbQuery, undefined, async function (err, data) {
		...
		if (distributed){
			...
			var ts_ds_table = "CREATE TABLE IF NOT EXISTS "+distName+"_time_series (date Date, fingerprint UInt64, labels String, name String) ENGINE = Distributed('"+dbName+"','"+dbName+"','time_series',fingerprint);"
			var sm_ds_table = "CREATE TABLE IF NOT EXISTS "+distName+"_samples (fingerprint UInt64, timestamp_ms Int64, value Float64, string String) ENGINE = Distributed('"+dbName+"','"+dbName+"','samples',fingerprint);"

		  	await ch.query(ts_ds_table, undefined, function(err,data){
				if (err) { console.log(err); process.exit(1); }
				else if (debug) console.log('Distributed Timeseries Table ready!', distName);
				return true;
			});
		  	await ch.query(sm_ds_table, undefined, function(err,data){
				if (err) { console.log(err); process.exit(1); }
				else if (debug) console.log('Distributed Samples Table ready!', distName);
				return true;
			});
		}
