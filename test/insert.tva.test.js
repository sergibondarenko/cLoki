...
const isInsertBenchmarkEnabled = () => process.env.BENCHMARK && process.env.INSERT_BENCHMARK_CLOUD &&
    parseInt(process.env.BENCHMARK) && parseInt(process.env.INSERT_BENCHMARK_CLOUD)
...
const sendPoints = async (amount, fromMs, toMs) => {
    ...
    try {
        await axios.post('https://15062:eyJrIjoiYTFlOWFkMDE4YjlhZDkyZDc1ZGU2OWMzM2Y0YTkyMmVkNjhjN2EyMCIsIm4iOiJjbG9raS1wdXNoIiwiaWQiOjE0NDU0fQ==@logs-prod-us-central1.grafana.net/loki/api/v1/push', {
            streams: Object.values(points)
        }, {
            headers:{"Content-Type": "application/json"}
        });
    } catch (e) {
        console.log("ERROR AXIOS", e.response);
        throw e;
    }
